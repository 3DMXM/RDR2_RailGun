...
//标题相关样式
const float
	MenuItemTitle_lineWidth	 = 0.22f,	//列宽
	MenuItemTitle_lineHeight = 0.06f,	//列高
	MenuItemTitle_textLeft	 = 0.01f;	//文本左侧距离

const ColorRgba
	MenuItemTitle_colorRect { 47, 53, 66, 255 },	//rgba(47, 53, 66,1.0)
	MenuItemTitle_colorText { 255, 255, 255, 255 };	//rgba(241, 242, 246,1.0)
...
//列表相关样式
const float
	MenuItemDefault_lineWidth	= 0.22f,	//列宽
	MenuItemDefault_lineHeight	= 0.04f,	//列高
	MenuItemDefault_textLeft	= 0.01f;	//文本左侧距离

const ColorRgba
	MenuItemDefault_colorRect			{ 116, 125, 140, 255 },		//默认背景颜色RGBA rgba(116, 125, 140,1.0)
	MenuItemDefault_colorText			{ 255, 255, 255, 255 },		//默认字体颜色RGBA

	MenuItemDefault_colorRectActive		{ 56, 173, 169, 255 },			//选中项背景颜色RGBA rgba(87, 96, 111,1.0) rgba(56, 173, 169,1.0)
	MenuItemDefault_colorTextActive		{ 0, 0, 0, 255 };		//选中项字体颜色RGBA rgba(12, 36, 97,1.0)
...
//class MenuItemIndex : public MenuItemDefault
//{
//	int	m_index;
//	int max_index;
//	int min_index;
//public:
//	MenuItemIndex(string caption, int index, int min, int max)
//		: MenuItemDefault(caption),
//		m_index(index), min_index(min), max_index(max) {}
//	virtual eMenuItemClass GetClass() { return eMenuItemClass::Numeral; }
//	
//	virtual void OnDraw(float lineTop, float lineLeft, bool active);
//	virtual void OnAdd() { m_index++; if (m_index >= max_index) m_index = max_index;}
//	virtual void OnCut() { m_index--; if (m_index <= min_index) m_index = min_index;}
//	void SetState(int index) { m_index = index; }
...
public:
	...
	{
		return IsKeyJustUp(VK_F5) || KEY_DOWN(VK_GAMEPAD_RIGHT_SHOULDER) && KEY_DOWN(VK_GAMEPAD_LEFT_SHOULDER);	//启动按键
		//return IsKeyJustUp(key_activate);	//启动按键
	}
	static MenuInputButtonState GetButtonState()
	{
		return {
			IsKeyDown(VK_NUMPAD5) || IsKeyDown(VK_RETURN) || KEY_DOWN(VK_GAMEPAD_A),
			IsKeyDown(VK_NUMPAD0) || IsKeyDown(VK_BACK) || KEY_DOWN(VK_GAMEPAD_B),
			IsKeyDown(VK_NUMPAD8) || IsKeyDown(VK_UP) || KEY_DOWN(VK_GAMEPAD_LEFT_THUMBSTICK_UP),
			IsKeyDown(VK_NUMPAD2) || IsKeyDown(VK_DOWN) || KEY_DOWN(VK_GAMEPAD_LEFT_THUMBSTICK_DOWN),
			IsKeyDown(VK_NUMPAD4) || IsKeyDown(VK_LEFT) || KEY_DOWN(VK_GAMEPAD_DPAD_LEFT),
			IsKeyDown(VK_NUMPAD6) || IsKeyDown(VK_RIGHT) || KEY_DOWN(VK_GAMEPAD_DPAD_RIGHT),
