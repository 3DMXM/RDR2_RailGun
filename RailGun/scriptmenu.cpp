...
void MenuItemBase::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	UI::SET_TEXT_SCALE(0.0, m_lineHeight * 7.0f);
...
void MenuItemSwitchable::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	//GetState() ? MenuOn.c_str() : MenuOff.c_str()
	DrawText(lineLeft + lineWidth - lineWidth / 6.35f, lineTop + lineHeight / 4.8f, GetState() ? "[开]" : "[关]");
}

////有数值的列表
//void MenuItemIndex::OnDraw(float lineTop, float lineLeft, bool active)
//{
//	MenuItemDefault::OnDraw(lineTop, lineLeft, active);
//	float lineWidth = GetLineWidth();
//	float lineHeight = GetLineHeight();
...
//	UI::SET_TEXT_DROPSHADOW(0, 0, 0, 0, 0);
//
//	char* intex = (char*)GetIndex();
//	std::string const& cc = string(">") + string(intex) + string("<");
//	intex = (char*)cc.c_str();
//	DrawText(lineLeft + lineWidth - lineWidth / 6.35f, lineTop + lineHeight / 4.8f, intex);
//}
...
int MenuBase::OnInput()
{
	...
	{
		/*if (MenuItemIndex::GetClass() == eMenuItemClass::Numeral)
		{
